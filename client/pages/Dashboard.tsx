...
export default function Dashboard() {
  ...
  const [viewMode, setViewMode] = useState<"freelancer" | "buyer">(
    "freelancer",
  );
  ...
  const [jobs] = useState<Job[]>([
    {
      ...
      description:
        "Build a complete e-commerce platform with React, Redux, and payment integration.",
      client: "TechCorp Inc.",
      freelancer: "John Doe",
      budget: 2500,
      status: "in_progress",
    ...
    },
    {
      ...
      description:
        "Create modern, user-friendly mobile app design for iOS and Android.",
      client: "StartupXYZ",
      budget: 800,
      status: "completed",
      category: "Design",
    ...
    },
    {
      ...
      description:
        "Simple blog website with custom theme and basic functionality.",
    ...
    },
    {
      ...
      description:
        "Create data analysis scripts for processing CSV files and generating reports.",
  ...
  const [messages] = useState<Message[]>([
    {
      ...
      to: "John Doe",
      content:
        "Great progress on the e-commerce project! The dashboard looks fantastic.",
  ...
  const freelancerStats = useMemo(() => {
    const freelancerJobs = jobs.filter(
      (job) => job.freelancer === currentUser.name,
    );
    const activeJobs = freelancerJobs.filter((job) =>
      ["active", "in_progress"].includes(job.status),
    );
    const completedJobs = freelancerJobs.filter(
    ...
    return {
      ...
      pendingJobs: freelancerJobs.filter((job) => job.status === "pending")
        .length,
    };
  }, [jobs, currentUser]);
  ...
  const buyerStats = useMemo(() => {
    const buyerJobs = jobs.filter(
      (job) =>
        job.client === currentUser.name ||
        job.client.includes("Corp") ||
        job.client.includes("Solutions"),
    );
    const activeJobs = buyerJobs.filter((job) =>
      ["active", "in_progress"].includes(job.status),
    );
    ...
    return {
      ...
      totalSpent,
      savedJobs: 5,
      applicationsReceived: buyerJobs.reduce(
  ...
  const filteredJobs = useMemo(() => {
    ...
    if (viewMode === "freelancer") {
      filtered = jobs.filter(
        (job) => job.freelancer === currentUser.name || !job.freelancer,
      );
    } else {
      filtered = jobs.filter(
        (job) =>
  ...
  const unreadMessages = messages.filter(
    (msg) => !msg.read && msg.to === currentUser.name,
  ).length;
              ...
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-2xl font-bold text-gray-900">
                      ${buyerStats.totalSpent.toLocaleString()}
                    </p>
              ...
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-2xl font-bold text-gray-900">
                      {buyerStats.applicationsReceived}
                    </p>
        ...
        <div className="lg:col-span-2">
          <Card>
            <CardHeader>
              <div className="flex items-center justify-between">
                <CardTitle>
                  Recent {viewMode === "freelancer" ? "Jobs" : "Projects"}
                </CardTitle>
                <Link
                      ...
                      <div className="flex items-start justify-between mb-3">
                        <div className="flex-1">
                          ...
                          <p className="text-sm text-gray-600">
                            {viewMode === "freelancer"
                              ? `Client: ${job.client}`
                              : `Freelancer: ${job.freelancer || "Not assigned"}`}
                          </p>
                        </div>
        ...
        {filteredJobs.length === 0 ? (
          <Card>
            <CardContent className="p-12 text-center">
              ...
              <Button asChild>
                <Link
                  to={viewMode === "freelancer" ? "/find-jobs" : "/post-job"}
                >
                  <Plus className="w-4 h-4 mr-2" />
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600">
            Track your earnings, expenses, and transactions
          </p>
        </div>
        <div className="flex gap-2">
